...
import (
	...
	"github.com/hamradiolog-net/adif-spec/src/pkgx/spec"
	"github.com/hamradiolog-net/adif-spec/src/pkgx/specdata"
)
...
func generate(spec spec.AdifSpec, records any, tmplName, packageName, typeName, constPrefix string) {
	fileName := filepath.Join("src/cmd/specgen2/template/", tmplName+".tmpl")
...
func writeToFile(dir, filename, content string) {
	fullPath := filepath.Join("src/pkgx", dir, filename)
