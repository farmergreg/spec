...
var lookupList = []Spec{
	...
	{Key: "DATE", DataTypeIndicator: 34, Description: "8 Digits representing a UTC date in YYYYMMDD format, where YYYY is a 4-Digit year specifier, where 1930 <= YYYY MM is a 2-Digit month specifier, where 1 <= MM <= 12 [use leading zeroes] DD is a 2-Digit day specifier, where 1 <= DD <= DaysInMonth(MM) [use leading zeroes]", MinimumValue: 0, MaximumValue: 0, IsImportOnly: false, Comments: ""},
	...
	{Key: "ENUMERATION", DataTypeIndicator: 34, Description: "an explicit list of legal case-insensitive values represented in ASCII set forth in set notation, e.g. {A, B, C, D}, or defined in a table, from which a single value may be selected.", MinimumValue: 0, MaximumValue: 0, IsImportOnly: false, Comments: ""},
	...
	{Key: "INTLMULTILINESTRING", DataTypeIndicator: 34, Description: "a sequence of International Characters and line breaks. Fields of type IntlMultilineString must only be used in ADX files", MinimumValue: 0, MaximumValue: 0, IsImportOnly: false, Comments: ""},
	{Key: "INTLSTRING", DataTypeIndicator: 34, Description: "a sequence of International Characters. Fields of type IntlString must only be used in ADX files", MinimumValue: 0, MaximumValue: 0, IsImportOnly: false, Comments: ""},
	{Key: "LOCATION", DataTypeIndicator: 34, Description: "a sequence of 11 characters representing a latitude or longitude in XDDD MM.MMM format, where X is a directional Character from the set {E, W, N, S} DDD is a 3-Digit degrees specifier, where 0 <= DDD <= 180 [use leading zeroes] There is a single space character in between DDD and MM.MMM MM.MMM is an unsigned Number minutes specifier with its decimal point in the third position, where 00.000 <= MM.MMM <= 59.999 [use leading and trailing zeroes]", MinimumValue: 0, MaximumValue: 0, IsImportOnly: false, Comments: ""},
	{Key: "MULTILINESTRING", DataTypeIndicator: 34, Description: "a sequence of Characters and line-breaks, where a line break is an ASCII CR (code 13) followed immediately by an ASCII LF (code 10)", MinimumValue: 0, MaximumValue: 0, IsImportOnly: false, Comments: ""},
	{Key: "NUMBER", DataTypeIndicator: 34, Description: "a sequence of one or more Digits representing a decimal number, optionally preceded by a minus sign (ASCII code 45) and optionally including a single decimal point (ASCII code 46)", MinimumValue: 0, MaximumValue: 0, IsImportOnly: false, Comments: ""},
	...
	{Key: "STRING", DataTypeIndicator: 34, Description: "a sequence of Characters", MinimumValue: 0, MaximumValue: 0, IsImportOnly: false, Comments: ""},
	{Key: "TIME", DataTypeIndicator: 34, Description: "6 Digits representing a UTC time in HHMMSS format or 4 Digits representing a time in HHMM format, where HH is a 2-Digit hour specifier, where 0 <= HH <= 23 [use leading zeroes] MM is a 2-Digit minute specifier, where 0 <= MM <= 59 [use leading zeroes] SS is a 2-Digit second specifier, where 0 <= SS <= 59 [use leading zeroes]", MinimumValue: 0, MaximumValue: 0, IsImportOnly: false, Comments: ""},
